...
-(void)askPermission {
    ...
    [alert setMessageText: [NSString stringWithFormat:@"ViFlow cần bạn cấp quyền để có thể hoạt động!"]];
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    if (!dontCheckUpdate)
        [ViFlowManager checkNewVersion:nil callbackFunc:nil];
...
-(void)onConvertTool {
    ...
    //[ViFlowManager showDockIcon:YES];
...
-(void)onQuickConvert {
    if ([ViFlowManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [ViFlowManager showMessage: nil message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [ViFlowManager showMessage: nil message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}


-(IBAction) onControlPanelSelected:(id)sender {
    [self onControlPanelSelected];
}

-(void) onControlPanelSelected {
    if (_mainWC == nil) {
        _mainWC = [[NSStoryboard storyboardWithName:@"Main" bundle:nil] instantiateControllerWithIdentifier:@"ViFlow"];
        _mainWC.window.delegate = self;
    }
    NSWindow *win = _mainWC.window;
    [win setLevel:NSFloatingWindowLevel];
    // If called from a menu item or other non-click context, ensure activation.
...
-(void)windowWillClose:(NSNotification *)notification {
    // When settings window closes, revert to Accessory (no Dock icon) unless user enabled Dock icon
    if (!vShowIconOnDock) {
        dispatch_async(dispatch_get_main_queue(), ^{
...
-(void)activeAppChanged: (NSNotification*)note {
    if (vUseSmartSwitchKey && [ViFlowManager isInited]) {
        OnActiveAppChanged();
    }
}
