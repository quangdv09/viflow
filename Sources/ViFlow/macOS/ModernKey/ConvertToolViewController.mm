...
-(void)fillData {
    NSArray* codeData = [ViFlowManager getTableCodes];
...
- (IBAction)onConvertButton:(id)sender {
    if ([ViFlowManager quickConvert]) {
        if (!convertToolDontAlertWhenCompleted) {
            [ViFlowManager showMessage: self.view.window message:@"Chuyển mã thành công!" subMsg:@"Kết quả đã được lưu trong clipboard."];
        }
    } else {
        [ViFlowManager showMessage: self.view.window message:@"Không có dữ liệu trong clipboard!" subMsg:@"Hãy sao chép một đoạn text để chuyển đổi!"];
    }
}
