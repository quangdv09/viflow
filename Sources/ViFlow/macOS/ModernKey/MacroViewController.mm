...
- (void)viewDidLoad {
    ...
    //load data
    getAllMacro(keys, macroText, macroContents);
}
...
- (IBAction)onLoadFromFile:(id)sender {
    ...
    [openPanel setNameFieldStringValue:@"ViFlowMacro"];
...
- (IBAction)onExportToFile:(id)sender {
    ...
    [savePanel setNameFieldStringValue:@"ViFlowMacro"];
...
- (nullable NSView *)tableView:(NSTableView *)tableView viewForTableColumn:(nullable NSTableColumn *)tableColumn row:(NSInteger)row {
    ...
    if (tableColumn == tableView.tableColumns[0]) {
        ...
        cellId = @"ContentCell";
        v = [tableView makeViewWithIdentifier:cellId owner:self];
        [v.textField setStringValue:[NSString stringWithUTF8String:macroContents[row].c_str()]];
    }
    return v;
}
...
- (BOOL)tableView:(NSTableView *)tableView shouldSelectRow:(NSInteger)row {
    ...
    [self.macroContent setStringValue:[NSString stringWithUTF8String:macroContents[row].c_str()]];
    [self.buttonAdd setTitle:MACRO_EDIT_TEXT];
    return YES;
}
