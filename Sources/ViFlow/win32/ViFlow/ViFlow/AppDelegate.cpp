...
int AppDelegate::run(HINSTANCE hInstance) {
	...
	while (GetMessage(&msg, nullptr, 0, 0))	{
		if (msg.message == WM_KEYDOWN) {
			ViFlowManager::_lastKeyCode = (UINT16)msg.wParam;
		}
		if (!isDialogMsg(msg)) {
...
void AppDelegate::onToggleVietnamese() {
	...
	if (vUseSmartSwitchKey) {
		string& exe = ViFlowHelper::getLastAppExecuteName();
...
void AppDelegate::onViFlowAbout() {
	if (aboutDialog == NULL) {
...
void AppDelegate::onViFlowExit() {
	ViFlowManager::freeEngine();
