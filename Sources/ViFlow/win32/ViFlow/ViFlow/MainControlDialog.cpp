...
void MainControlDialog::initDialog() {
    ...
    wsprintfW(titleBuffer, title, ViFlowHelper::getVersionString().c_str());
    SetWindowText(hDlg, titleBuffer);

    //input type
    comboBoxInputType = GetDlgItem(hDlg, IDC_COMBO_INPUT_TYPE);
    vector<LPCTSTR>& inputType = ViFlowManager::getInputType();
    ...
    vector<LPCTSTR>& tableCode = ViFlowManager::getTableCode();
    ...
    checkTempOffViFlow = GetDlgItem(hTabPage1, IDC_CHECK_TEMP_OFF_VIFLOW);
    createToolTip(checkTempOffViFlow, IDS_STRING_TEMP_OFF_VIFLOW);
...
INT_PTR MainControlDialog::eventProc(HWND hDlg, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_COMMAND: {
        ...
        case IDBUTTON_OK:
            ...
            break;
        case IDBUTTON_EXIT:
            AppDelegate::getInstance()->onViFlowExit();
            break;
        case ID_BTN_DEFAULT: {
        ...
        case IDC_BUTTON_GO_SOURCE_CODE:
            ShellExecute(NULL, _T("open"), _T("https://github.com/quangdv09/viflow"), NULL, NULL, SW_SHOWNORMAL);
            break;
        default:
            if (HIWORD(wParam) == CBN_SELCHANGE) {
                ...
                _lastKeyCode = ViFlowManager::_lastKeyCode;
                if (_lastKeyCode > 0) {
                    ViFlowManager::_lastKeyCode = 0;
...
void MainControlDialog::fillData() {
    ...
    SendMessage(checkTempOffViFlow, BM_SETCHECK, vTempOffViFlow ? 1 : 0, 0);
    ...
    //tab info
    wchar_t buffer[256];
    wsprintfW(buffer, _T("Phiên bản %s cho Windows - Ngày cập nhật: %s"), ViFlowHelper::getVersionString().c_str(), _T(__DATE__));
...
void MainControlDialog::onComboBoxSelected(const HWND& hCombobox, const int& comboboxId) {
    if (hCombobox == comboBoxInputType) {
        ...
        if (vRememberCode) {
            setAppInputMethodStatus(ViFlowHelper::getFrontMostAppExecuteName(), vLanguage | (vCodeTable << 1));
...
void MainControlDialog::onCheckboxClicked(const HWND& hWnd) {
    ...
    if (hWnd == checkCtrl) {
        ...
        ViFlowHelper::registerRunOnStartup(vRunWithWindows);
    }
        ...
        if (val)
            ViFlowManager::createDesktopShortcut();
    }
    ...
    else if (hWnd == checkTempOffViFlow) {
        val = (int)SendMessage(hWnd, BM_GETCHECK, 0, 0);
        APP_SET_DATA(vTempOffViFlow, val ? 1 : 0);
    }
...
void MainControlDialog::onUpdateButton() {
    ...
    string newVersion;
    if (ViFlowManager::checkUpdate(newVersion)) {
        WCHAR msg[256];
        wsprintf(msg,
            TEXT("ViFlow Có phiên bản mới (%s), bạn có muốn cập nhật không?"),
        ...
        if (msgboxID == IDYES) {
            ...
            wsprintf(path, TEXT("%s\\ViFlowUpdate.exe"), path);
        ...
        MessageBox(hDlg, _T("Bạn đang dùng phiên bản mới nhất!"), _T("ViFlow Update"), MB_OK);
    }
    EnableWindow(hUpdateButton, true);
}

void MainControlDialog::requestRestartAsAdmin() {
    ViFlowHelper::registerRunOnStartup(false);
    if (vRunAsAdmin && !IsUserAnAdmin()) {
        int msgboxID = MessageBox(
            hDlg,
            _T("Bạn cần phải khởi động lại ViFlow để kích hoạt chế độ Admin!\nBạn có muốn khởi động lại ViFlow không?"),
            _T("ViFlow"),
            MB_ICONEXCLAMATION | MB_YESNO
        );
        if (msgboxID == IDYES) {
            PostQuitMessage(0);
            ShellExecute(0, L"runas", ViFlowHelper::getFullPath().c_str(), 0, 0, SW_SHOWNORMAL);
        }
    }
    else {
        ViFlowHelper::registerRunOnStartup(vRunWithWindows);
