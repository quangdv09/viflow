...
static LPCTSTR sk = TEXT("SOFTWARE\\QuangDV09\\ViFlow");
static HKEY hKey;
...
static HANDLE _proc;
static string _exeNameUtf8 = "ViFlow";
...
int CF_VIFLOW = RegisterClipboardFormat(_T("ViFlow Format"));

void ViFlowHelper::openRegistryKey() {
...
void ViFlowHelper::setRegInt(LPCTSTR key, const int & val) {
	openRegistryKey();
	RegSetValueEx(hKey, key, 0, REG_DWORD, (LPBYTE)&val, sizeof(val));
	RegCloseKey(hKey);
}

int ViFlowHelper::getRegInt(LPCTSTR key, const int & defaultValue) {
	openRegistryKey();
...
void ViFlowHelper::setRegBinary(LPCTSTR key, const BYTE * pData, const int & size) {
	openRegistryKey();
	RegSetValueEx(hKey, key, 0, REG_BINARY, pData, size);
	RegCloseKey(hKey);
}

BYTE * ViFlowHelper::getRegBinary(LPCTSTR key, DWORD& outSize) {
	openRegistryKey();
	if (_regData) {
		delete[] _regData;
		_regData = NULL;
...
void ViFlowHelper::registerRunOnStartup(const int& val) {
	if (val) {
		if (vRunAsAdmin) {
			string path = wideStringToUtf8(getFullPath());
			char buff[MAX_PATH];
			sprintf_s(buff, "schtasks /create /sc onlogon /tn ViFlow /rl highest /tr \"%s\" /f", path.c_str());
			WinExec(buff, SW_HIDE);
		} else {
			RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
			wstring path = getFullPath();
			RegSetValueEx(hKey, _T("ViFlow"), 0, REG_SZ, (byte*)path.c_str(), ((DWORD)path.size() + 1) * sizeof(TCHAR));
			RegCloseKey(hKey);
		}
	} else {
		RegOpenKeyEx(HKEY_CURRENT_USER, _runOnStartupKeyPath, NULL, KEY_ALL_ACCESS, &hKey);
		RegDeleteValue(hKey, _T("ViFlow"));
		RegCloseKey(hKey);
		WinExec("schtasks /delete  /tn ViFlow /f", SW_HIDE);
	}
}

LPTSTR ViFlowHelper::getExecutePath() {
	if (!_hasGetPath) {
...
string& ViFlowHelper::getFrontMostAppExecuteName() {
	...
	if (wcscmp(_exeName, _T("ViFlow64.exe")) == 0 ||
		wcscmp(_exeName, _T("ViFlow32.exe")) == 0 || 
...
string & ViFlowHelper::getLastAppExecuteName() {
	if (!vUseSmartSwitchKey)
		return getFrontMostAppExecuteName();
	return _exeNameUtf8;
}

wstring ViFlowHelper::getFullPath() {
...
wstring ViFlowHelper::getClipboardText(const int& type) {
...
void ViFlowHelper::setClipboardText(LPCTSTR data, const int & len, const int& type) {
...
bool ViFlowHelper::quickConvert() {
...
DWORD ViFlowHelper::getVersionNumber() {
...
wstring ViFlowHelper::getVersionString() {
...
wstring ViFlowHelper::getContentOfUrl(LPCTSTR url){
	WCHAR path[MAX_PATH];
	GetTempPath2(MAX_PATH, path);
	wsprintf(path, TEXT("%s\\_ViFlow.tempf"), path);
